...
import (
	...
	"github.com/zhucanyi/go_seckill/service"
	"math/rand"
	"net/http"
	"net/url"
	"os"
...
func (this *Seckill) getUserAgent() string {
	...
	randEnable := this.conf.MustValue("config", "rand", "false")
	if randEnable == "true" {
		i := rand.Intn(50)
		log.Debug("rand User agent:", i, USER_AGENTS[i])
